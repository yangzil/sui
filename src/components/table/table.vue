<template>
  <div class="s-table" :class="[
    stripe ? 's-table--stripe' : '',
    border ? 's-table--border' : '',
    highlightCurrentRow ? 'is-highlight' : ''
  ]" ref="container" v-s-loading="loading">
    <!--main columns-->
    <div class="s-table__hidden" v-show="false"><slot/></div>
    <div class="s-table__header-wrapper" ref="table_header" @scroll="tableScrollHandler('header')">
      <table cellspacing="0" cellpadding="0" border="0" class="s-table__header" :style="{
        width: !isBeyondWidth? `${width}px` : `${rowWidth}px`,
        overflowX: !isBeyondWidth ? 'hidden' : ''
      }">
        <tr class="s-table__header-row">
          <th v-for="(c, idx) in columns" :key="idx" :style="{
            width: c.width ? `${c.width}px` : `${averageWidth}px`
          }">
            <div class="s-table__cell" :style="{
              width: c.width ? `${c.width}px` : `${averageWidth}px`
            }">
              ...
              <span v-if="c.sortMethod" class="s-table__sort"><i class="fa fa-caret-up" @click="sortColumn(c.sortMethod)"></i><i class="fa fa-caret-down" @click="sortColumn(c.sortMethod, 'desc')"></i></span>
            </div>
          </th>
        </tr>
      </table>
    ...
    <div class="s-table__body-wrapper" :style="{
      ...
      <table cellspacing="0" cellpadding="0" border="0" class="s-table__body" :style="{
        width: !isBeyondWidth? `${width}px` : `${rowWidth}px`
      }">
        <template v-for="(d, idx) in data">
          <tr :key="idx" class="s-table__body-row" :class="[
            rowClasses[idx],
            ...
            <td v-for="(c, idx2) in columns" :key="idx2" :style="{
              ...
              <template v-if="c.type === 'selection'">
                <div class="s-table__cell">
                  <t-checkbox v-model="selectionRows" :val="d" @change="selectionRowChange"/>
                </div>
              </template>
              <template v-else>
                <s-table-cell :row="d" :col="c" :idx="idx" :width="c.width ? `${c.width}px` : `${averageWidth}px`"
          ...
          <!--<transition name="fade" :key="idx">-->
            <tr class="s-table__body-row" :key="-idx-1" v-if="expandColumn && expandIdx === idx">
              <td :style="{
                display: 'inline-table'
              }">
                <s-table-expand :row="d" :col="expandColumn" :width="(width) + 'px'"/>
              </td>
            </tr>
          <!--</transition>-->
        </template>
    ...
    <!--right fixed columns-->
    <div class="s-table__fixed" :style="{
      width: `${leftFixedWidth}px`
    }" :class="[
      scrollLeft === 0 ? '' : 'is-shadow'
    ]" v-if="tableWidth > width">
      <div class="s-table__fixed-header" ref="table_fixed_header" :style="{
        overflowX: 'hidden'
      }">
        <table cellspacing="0" cellpadding="0" border="0" class="s-table__header" :style="{
          width: `${leftFixedWidth}px`
      ...
      <div class="s-table__fixed-body" :style="{
        height: bodyHeight ? `${bodyHeight}px` : 'auto',
        maxHeight: bodyMaxHeight ? `${bodyMaxHeight}px` : '',
        overflowX: 'hidden'
    ...
    <!--right fixed columns-->
    <div class="s-table__fixed-right" :style="{
      width: `${rightFixedWidth}px`
    }" :class="[
      scrollLeft >= tableWidth - width - 1 ? '' : 'is-shadow'
    ]" v-if="tableWidth > width">
      <div class="s-table__fixed-header" ref="table_fixed_right_header" :style="{
        overflowX: 'hidden',
        marginLeft: rightFit
      }">
        <table cellspacing="0" cellpadding="0" border="0" class="s-table__header" :style="{
